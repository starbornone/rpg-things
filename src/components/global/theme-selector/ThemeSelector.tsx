...
export function ThemeSelector() {
  ...
  return (
    <Listbox as="div" className="relative z-40" value={theme} onChange={setTheme}>
      ...
      <ListboxButton
        ...
        aria-label="Theme"
      >
        <LightIcon className={clsx('h-4 w-4 dark:hidden', theme === 'system' ? 'fill-gray-400' : 'fill-cyan-400')} />
        <DarkIcon
          className={clsx('hidden h-4 w-4 dark:block', theme === 'system' ? 'fill-gray-400' : 'fill-cyan-400')}
        />
      </ListboxButton>
      <ListboxOptions
        anchor="bottom start"
            ...
            {({ selected }) => (
              <>
                <div className="rounded-md bg-white p-1 shadow ring-1 ring-gray-900/5 dark:bg-gray-700 dark:ring-inset dark:ring-white/5">
                  <theme.icon
                    className={clsx('h-4 w-4', selected ? 'fill-cyan-400 dark:fill-cyan-400' : 'fill-gray-400')}
                  />
                </div>
